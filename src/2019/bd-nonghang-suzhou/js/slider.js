    ...
    $(".products li").each(function(i,el){
     ...
     /* totalPrice = currentItem.ids[0]; /!*总价格*!/
      ...
      if(currentItem.ponds.length==1){
        totalPrice = currentItem.ids[0];
        oldPrice = totalPrice-0;
        totalPrice = isDiscount(totalPrice,0,discount,zhekou);
        $(this).find(".bang").html(currentItem.ponds[0]);
        $(this).find(".dis-price").html(totalPrice.toFixed(2));
        $(this).find(".old-price").html(oldPrice.toFixed(2));
        $(this).find(".postid").data("postid", currentItem.postId[0]);
        ...
        currentItem.ix=1;
        totalPrice = currentItem.ids[1];
        oldPrice = totalPrice-0;
        
        $(".products li").each(function () {
          $(this).find(".select-pond dd").eq(1).addClass("cur").siblings().removeClass("cur");
        });

        totalPrice = isDiscount(totalPrice,1,discount,zhekou);
